...
const playlist = []
let current = -1
const iconp = document.querySelector('#btn_play > ion-icon')
...
checkInternetConnected(checkInternetConnectedConfig).then(() => {
	...
	accname.innerText = 'درحال نمایش نیست DiscordRPC به دلیل عدم اتصال به اینترنت'
});
...
const handelPlayPause = () => {
	if (playlist.length) {
		if (player.paused) {
			player.play()
			iconp.name = 'pause-outline'
			ipcRenderer.send('setStatus', 'نگه‌داشتن')
		} else {
			player.pause()
			iconp.name = 'play-outline'
